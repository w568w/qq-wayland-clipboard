...
| --- | --- | --- |
| 纯文本 | 原样转发 QQ 私有富文本和 fallbacks | 由 QQ 原生读取 |
| 图片 | 转发 PNG；识别 magic header，将错误标记为 PNG 的 JPEG、BMP、GIF、ICO、TIFF 和 WebP 重新编码为 PNG | 由 QQ 原生读取 |
| 文件 | 将 QQ 私有文件元素转换为标准 `text/uri-list` | 由 QQ 原生读取 |
...
- Wayland data-control 不提供条件更新 selection 的原子操作。目前 bridge 会通过 Generation Ticking 以缩小竞态窗口，但无法保证与另一个应用同时复制时的绝对顺序。

## 6. 许可

本项目以 GNU 通用许可证版本 v3.0 或更高 发布。
